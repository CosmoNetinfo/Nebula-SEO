                ...
                {activeTab === 'seo' && (
                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-300 space-y-6">
                        ...
                        <div className="space-y-4">
                            <div className="bg-slate-900/40 p-4 rounded-xl border border-indigo-500/10">
                                <label className="text-[10px] uppercase font-bold text-indigo-400 block mb-2">Categorie Editoriali</label>
                                <p className="text-sm text-slate-200">{result.categories}</p>
                            </div>
                            
                            <div className="bg-slate-900/40 p-4 rounded-xl border border-indigo-500/10">
                                <div className="flex justify-between items-center mb-3">
                                    <label className="text-[10px] uppercase font-bold text-indigo-400">Tag SEO (CSV)</label>
                                    <button onClick={handleCopyTagsCSV} className="text-[10px] text-slate-400 hover:text-indigo-400 flex items-center gap-1 font-bold">
                                        {tagsCopied ? <CheckCircleIcon className="w-3.5 h-3.5 text-green-400" /> : <ClipboardIcon className="w-3.5 h-3.5" />}
                                        Copia CSV
                                    </button>
                                </div>
                                <div className="bg-black/40 p-3 rounded-lg border border-slate-700 font-mono text-[10px] text-indigo-300/90 break-all cursor-pointer hover:border-indigo-500/30 transition-all" onClick={handleCopyTagsCSV}>
                                    {result.tags}
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-300 space-y-6">
                        <div className="flex items-center justify-between">
                            <div className="flex bg-slate-900 p-1 rounded-lg border border-slate-700">
                                <button onClick={() => setViewMode('preview')} className={`px-4 py-1.5 text-[10px] font-bold rounded-md transition-all ${viewMode === 'preview' ? 'bg-indigo-600 text-white' : 'text-slate-500 hover:text-slate-300'}`}>ANTEPRIMA</button>
                                <button onClick={() => setViewMode('code')} className={`px-4 py-1.5 text-[10px] font-bold rounded-md transition-all ${viewMode === 'code' ? 'bg-indigo-600 text-white' : 'text-slate-500 hover:text-slate-300'}`}>HTML</button>
                            </div>
                            <div className="flex gap-2">
                                ...
                                <button onClick={() => window.print()} className="bg-slate-700 p-2.5 rounded-xl text-white hover:bg-slate-600 border border-slate-600" title="Stampa"><PrinterIcon className="w-5 h-5" /></button>
                            </div>
                        </div>
