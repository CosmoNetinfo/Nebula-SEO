...
export const LoadModal: React.FC<LoadModalProps> = ({ isOpen, onClose, articles, onLoad, onDelete }) => {
    ...
    return (
        ...
        >
            <div 
                className="bg-zinc-950 border border-zinc-800 rounded-xl shadow-2xl w-full max-w-2xl max-h-[80vh] flex flex-col slide-in-up"
                onClick={(e) => e.stopPropagation()}
            >
                <header className="flex items-center justify-between p-5 border-b border-zinc-900 bg-zinc-950">
                ...
                <div className="overflow-y-auto p-4 custom-scrollbar bg-black/40">
                    {articles.length === 0 ? (
                                ...
                                <li key={article.id} className="bg-zinc-900/40 p-4 rounded-xl border border-zinc-800/50 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 hover:border-zinc-700 hover:bg-zinc-900/60 transition-all group">
                                    ...
                                    <div className="flex items-center gap-2 flex-shrink-0 self-end sm:self-center opacity-80 group-hover:opacity-100 transition-opacity">
                                        <button 
                                            ...
                                            className="px-4 py-2 text-[10px] font-bold uppercase tracking-wider text-black bg-white hover:bg-zinc-200 rounded-lg transition-colors border-none shadow-sm"
                                        >
                                            Load
                                        </button>
                                        <button 
