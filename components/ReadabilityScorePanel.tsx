    ...
    return (
        <div className="bg-black/60 p-5 rounded-2xl border border-zinc-700/50 shadow-lg relative overflow-hidden group mb-6">
            <div className="flex items-center justify-between mb-4 relative z-10">
                <h4 className="text-xs uppercase font-bold text-zinc-500 tracking-wider">Punteggio Leggibilità</h4>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 relative z-10">
                {items.map((r, i) => (
                     <div key={i} className="flex items-start gap-3 p-2 rounded-lg hover:bg-zinc-800/20 transition-colors">
                        ...
                        <div>
                            <p className="text-[10px] uppercase font-bold text-zinc-500">{r.criteria}</p>
                            <div className="flex items-baseline gap-2">
                                <span className="text-xs font-bold text-zinc-200">{r.score}</span>
                            </div>
                            <p className="text-[10px] text-zinc-400 leading-tight mt-1">{r.message}</p>
                        </div>
                    </div>
                ))}
            </div>
