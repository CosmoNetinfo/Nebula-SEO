    ...
    return (
        <div className="bg-zinc-950/30 p-5 rounded-2xl border border-zinc-800/50 shadow-lg relative overflow-hidden group mb-6">
            <div className="flex items-center justify-between mb-4 relative z-10">
                <h4 className="text-xs uppercase font-bold text-zinc-500 tracking-wider">Readability Score</h4>
                ...
                {items.map((r, i) => (
                     <div key={i} className="flex items-start gap-3 p-3 rounded-xl border border-zinc-800/30 bg-black/20 hover:bg-zinc-900/40 transition-colors group/item">
                        <div className="mt-0.5 shrink-0">
                            {r.status === 'good' && <CheckCircleIcon className="w-4 h-4 text-zinc-300" />}
                            {r.status === 'average' && <ExclamationTriangleIcon className="w-4 h-4 text-zinc-500" />}
                            {r.status === 'poor' && <XCircleIcon className="w-4 h-4 text-zinc-600" />}
                        </div>
                        
                        <div className="flex-1">
                            <div className="flex justify-between items-baseline mb-1">
                                <p className="text-[10px] uppercase font-bold text-zinc-500 group-hover/item:text-zinc-400 transition-colors">{r.criteria}</p>
                                <span className="text-[10px] font-bold text-zinc-300 font-mono bg-zinc-900/50 px-1.5 rounded">{r.score}</span>
                            </div>
                            <p className="text-[10px] text-zinc-400 leading-tight font-mono">{r.message}</p>
                        </div>
                    </div>
                ))}
            </div>
